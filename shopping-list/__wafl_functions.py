...
def add_shopping_list(item, inference, task_memory):
    ...
    if not {
        inference.get_inference_answer(f" The user adds {item} to a list :- the user adds something to a grocery list ", task_memory)
    }:
...
def remove_from_shopping_list(item, inference, task_memory):
    shopping_list = json.load(open("shopping_list.json"))
    if not shopping_list:
        inference.get_inference_answer(f" SAY the shopping list is already empty.", task_memory)
        return False
